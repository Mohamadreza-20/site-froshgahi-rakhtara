  ...
  return <div className="space-y-6">
    ...
    <div className="bg-white rounded-2xl border border-[#EEF0F5] p-5 shadow-sm shadow-black/[0.02] space-y-5">
      {state.error ? <QueryErrorState message="خطا در دریافت محصولات" onRetry={state.refetch} /> : state.loading ? <QueryLoadingState message="در حال بارگذاری محصولات..." /> : <>
        <ProductsToolbar query={state.query} onQueryChange={state.setQuery} activeCategory={state.activeCategory} onCategoryChange={state.setActiveCategory} view={state.view} onViewChange={state.setView} />
        {state.fetching && <p className="text-xs text-ink/40" aria-live="polite">در حال به‌روزرسانی نتایج…</p>}
